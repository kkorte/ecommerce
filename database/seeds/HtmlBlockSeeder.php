...
class HtmlBlockTableSeeder extends Seeder
{
    public function run()
    {
        ...
        if (! $htmlBlock->save()) {
            Log::info('Unable to html block '.$htmlBlock->title, (array)$htmlBlock->errors());
        } else {
            Log::info('Created html block "'.$htmlBlock->title.'" <'.$htmlBlock->title.'>');     
        }
        ...
        if (! $htmlBlock2->save()) {
            Log::info('Unable to html block '.$htmlBlock2->title, (array)$htmlBlock2->errors());
        } else {
            Log::info('Created html block "'.$htmlBlock2->title.'" <'.$htmlBlock2->title.'>');     
        }
