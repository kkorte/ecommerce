...
class CouponTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(config('hideyo.db_prefix').'coupon', function (Blueprint $table) {
            $table->increments('id');
        ...
        Schema::create(config('hideyo.db_prefix').'coupon_group', function ($table) {
            $table->increments('id');
            $table->boolean('active')->default(false);
            $table->string('title');
        ...
        Schema::table(config('hideyo.db_prefix').'coupon', function (Blueprint $table) {
            $table->integer('coupon_group_id')->unsigned()->nullable();
            $table->foreign('coupon_group_id')->references('id')->on(config('hideyo.db_prefix').'coupon_group')->onDelete('set null');
        });

        Schema::create(config('hideyo.db_prefix').'coupon_product', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('coupon_id');
            $table->foreign('coupon_id')->references('id')->on(config('hideyo.db_prefix').'coupon')->onDelete('cascade');
        ...
        Schema::create(config('hideyo.db_prefix').'coupon_product_category', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('coupon_id');
            $table->foreign('coupon_id')->references('id')->on(config('hideyo.db_prefix').'coupon')->onDelete('cascade');
        ...
        Schema::create(config('hideyo.db_prefix').'coupon_sending_method', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('coupon_id');
            $table->foreign('coupon_id')->references('id')->on(config('hideyo.db_prefix').'coupon')->onDelete('cascade');
            $table->unsignedInteger('sending_method_id');
            $table->foreign('sending_method_id')->references('id')->on(config('hideyo.db_prefix').'sending_method')->onDelete('cascade');
            $table->unique(array('coupon_id','sending_method_id'), 'unique_sending_method_id');
            $table->timestamps();
        });

        Schema::create(config('hideyo.db_prefix').'coupon_payment_method', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('coupon_id');
            $table->foreign('coupon_id')->references('id')->on(config('hideyo.db_prefix').'coupon')->onDelete('cascade');
            $table->unsignedInteger('payment_method_id');
            $table->foreign('payment_method_id')->references('id')->on(config('hideyo.db_prefix').'payment_method')->onDelete('cascade');
