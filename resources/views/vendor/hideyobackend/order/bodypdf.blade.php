	...
	<div class="contact">
		...
		<br/>
		Ordernummer: {{ $order->generated_custom_order_id }} ({{ $order->client->orders->count() }})
	...
	<table class="products" cellspacing="0" cellpadding="2">
		...
		<tr>
            ...
            <td>
                @if($product->original_price_with_tax != $product->price_with_tax AND $product->original_price_with_tax != 0)
                &#0128; {{ $product->getPriceWithTaxNumberFormat() }}
                <br/><small>korting
                {{ number_format((($product->original_price_with_tax - $product->price_with_tax) / $product->original_price_with_tax) * 100)  }}%
                <br/>orginele prijs: &#0128; {{ $product->getOriginalPriceWithTaxNumberFormat() }}
                </small>
                @else
                &#0128; {{ $product->getPriceWithTaxNumberFormat() }}
                @endif                                          
            </td>
			<td>&#0128; {{ $product->getTotalPriceWithTaxNumberFormat() }}</td>
		</tr>
		@endforeach
	</table>
	...
	<table class="details" cellspacing=0 cellpadding="2">
		...
		<tr>
			...
			<td class="color">&#0128; {{ $order->orderSendingMethod->getPriceWithTaxNumberFormat() }}</td>
		</tr>

		<tr>
			<td colspan="3" class="left"><strong>totaal:</strong></td>
			<td class="color">&#0128; {{ $order->getPriceWithTaxNumberFormat() }}</td>
		</tr>
