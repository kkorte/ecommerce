...
class ProductExtraFieldValueRepository implements ProductExtraFieldValueRepositoryInterface
{
    ...
    public function create(array $attributes, $productId)
    {
        ...
        if (isset($attributes['rows'])) {
            foreach ($attributes['rows'] as $row) {
                ...
                $data['shop_id'] = \Auth::guard('admin')->user()->selected_shop_id;
    ...
    {
        return $this->model->where('shop_id', '=', \Auth::guard('admin')->user()->selected_shop_id)->where('product_id', '=', $productId)->get();
    }

    public function selectByProductIdAndExtraFieldId($productId, $extraFieldId)
    {

        return $this->model->where('shop_id', '=', \Auth::guard('admin')->user()->selected_shop_id)->where('product_id', '=', $productId)->where('extra_field_id', '=', $extraFieldId)->get();
    }
