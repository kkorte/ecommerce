    ...
    public function getItem(Request $request, $slug)
    {
        ...
        if ($category) {
            ...
            if ($category->productCategoryImages->count()) {
                $og->image('/files/product_category/200x200/'.$category->productCategoryImages->first()->product_category_id.'/'.$category->productCategoryImages->first()->file);
            }
            ...
            if ($products) {
                $layers = $this->product->selectAllByShopIdAndProductCategoryIdDataLayer($this->shopId, $category['id'])->toArray();
